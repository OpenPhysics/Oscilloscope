...
import { OscilloscopeScreen } from "./oscilloscope-screen/OscilloscopeScreen.js";
import { OscilloscopePreferencesModel } from "./preferences/OscilloscopePreferencesModel.js";
import { OscilloscopePreferencesNode } from "./preferences/OscilloscopePreferencesNode.js";

onReadyToLaunch(() => {
  const stringManager = StringManager.getInstance();

  // Simulation-specific preferences; initial values come from oscilloscopeQueryParameters.
  const simPreferences = new OscilloscopePreferencesModel(Tandem.ROOT.createTandem("preferences"));
  ...
  const sim = new Sim(stringManager.getTitleStringProperty(), screens, {
    preferencesModel: new PreferencesModel({
      ...
      simulationOptions: {
        customPreferences: [
          {
            createContent: (tandem: Tandem) => new OscilloscopePreferencesNode(simPreferences, tandem),
          },
        ],
      },
      localizationOptions: {
